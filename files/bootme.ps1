...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include profile_mcollective"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/rely-profile_mcollective/master/files/bootme.ps1')) -ArgumentList ("profile_mcollective")
#</powershell>


  param(
    [string]$role = "profile_mcollective"
  )

  $puppet_source = "https://github.com/relybv/rely-profile_mcollective.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\profile_mcollective" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\profile_mcollective" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\profile_mcollective"
  }
